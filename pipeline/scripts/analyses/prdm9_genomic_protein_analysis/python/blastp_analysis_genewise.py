...
with open(f"results/{accession}/Step4_Hmm/blastp.txt", 'w') as writer:
    taxid = None
    string = ''
    os.system(f"mkdir -p data/assemblies/{accession}/SET_sequences/")
    os.system(f"mkdir -p data/assemblies/{accession}/SET_blastp/")
    for index, row in df.iterrows():
        ...
        print(f"Run blastdbcmd -db results/{accession}/Step3_genewise/protdb -entry {row['SeqID']} -range {int(row['SET domain start'])}-{int(row['SET domain end'])} -out data/assemblies/{accession}/SET_sequences/{row['SeqID']}.fa")
    ...
    df.to_csv(f"results/{accession}/Result_tables/summary_table_{accession}.csv", sep=';')
    if taxid == None:
        print("Nothing found in blastp")
    else:
        if string == '':
            string = f'<\t0\t0\t0\t0\n{accession}-ERROR\tNone\n'
